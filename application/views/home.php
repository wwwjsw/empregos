<!--
PAGINA INICIAL
-->	
<div class="row"> 	
	<div class="col s12 m6 l3">
		<div class="card medium grey darken-3">
			<div class="card-content white-text">
				<span class="card-title">Diarista</span>
				<div class="row">
					<div class="col l6 s6 lateral-text">
						<p>C.L.T</p>
						<p>Diaria</p>
						<p>Seguro de vida</p>
						<p>Plano odontológico</p>
					</div>
					<div class="col l6 s6 lateral-text">
						<p>Vale alimentação</p>
						<p>Plano de saúde</p>
						<p>Comissão</p>
						<p>Vale transporte</p>						
					</div>
				</div>
				<div class="row">
					<div class="col s12 l12 center-align">
					<a href=""><img src="assets/images/facebook.png" alt=""></a>
					<a href=""><img src="assets/images/twitter.png" alt=""></a>
					<a href=""><img src="assets/images/g_plus.png" alt=""></a>
					<a href=""><img src="assets/images/linkedin.png" alt=""></a>
					</div>
				</div>
			</div>
			<div class="card-action">
				<div class="row">
					<div class="col s12 l12 center-align margin-number">
						<a href="#" class="white-text">(00) 00000-0000</a>						
					</div>
					<div class="col s12 l12 center-align">
						<div class="chip "><img src="http://placehold.it/50x50/000000" alt="Contact Person">Guilherme Henrique</div>
					</div>
				</div>
				<div class="left-align white-text patrocinado-text"><p>Anúncio patrocinado</p></div>
			</div>
		</div>	    		
	</div>
	<div class="col s12 m6 l6">
		<div class="card medium orange darken-3">
			<div class="card-content white-text">
				<span class="card-title">Aux. de escritorio(LINK DIRETO)</span>
				<div class="row">
					<div class="col s6 l6">
						<p>C.L.T</p>
						<p>Beneficios</p>
						<p>Plano de saúde</p>
						<p>Plano odontológico</p>
					</div>
					<div class="col s6 l6">
						<p>C.L.T</p>
						<p>Beneficios</p>
						<p>Plano de saúde</p>
						<p>Plano odontológico</p>						
					</div>
				</div>
				<div class="row">
					<div class="col s12 l12 center-align">
					<a href=""><img src="assets/images/facebook.png" alt=""></a>
					<a href=""><img src="assets/images/twitter.png" alt=""></a>
					<a href=""><img src="assets/images/g_plus.png" alt=""></a>
					<a href=""><img src="assets/images/linkedin.png" alt=""></a>
					</div>
				</div>				
			</div>
			<div class="card-action">
				<div class="row">
					<div class="col s12 l12 center-align margin-number">
						<a href="#" class="white-text">(00) 00000-0000</a>						
					</div>
					<div class="col s12 l12 center-align">
						<div class="chip "><img src="http://placehold.it/50x50/000000" alt="Contact Person">Guilherme Henrique</div>
					</div>
				</div>
				<div class="left-align white-text patrocinado-text"><p>Anúncio patrocinado</p></div>
			</div>
		</div>	    		
	</div>
	<div class="col s12 m6 l3">
		<div class="card medium red darken-3">
			<div class="card-content white-text">
				<span class="card-title">Diarista</span>
				<div class="row">
					<div class="col l6 s6 lateral-text">
						<p>C.L.T</p>
						<p>Beneficios</p>
						<p>Plano de saúde</p>
						<p>Plano odontológico</p>
					</div>
					<div class="col l6 s6 lateral-text">
						<p>C.L.T</p>
						<p>Beneficios</p>
						<p>Plano de saúde</p>
						<p>Plano odontológico</p>						
					</div>
				</div>
				<div class="row">
					<div class="col s12 l12 center-align">
					<a href=""><img src="assets/images/facebook.png" alt=""></a>
					<a href=""><img src="assets/images/twitter.png" alt=""></a>
					<a href=""><img src="assets/images/g_plus.png" alt=""></a>
					<a href=""><img src="assets/images/linkedin.png" alt=""></a>
					</div>
				</div>
			</div>
			<div class="card-action">
				<div class="row">
					<div class="col s12 l12 center-align margin-number">
						<a href="#" class="white-text">(00) 00000-0000</a>						
					</div>
					<div class="col s12 l12 center-align">
						<div class="chip "><img src="http://placehold.it/50x50/000000" alt="Contact Person">Guilherme Henrique</div>
					</div>
				</div>
				<div class="left-align white-text patrocinado-text"><p>Anúncio patrocinado</p></div>
			</div>
		</div>	    		
	</div>
</div>
<!--
FINAL PAGINA INICIAL
-->
<!-- 
	INICIO DA PAGINAÇÃO
-->
<div class="row">
	<ul class="pagination center-align">
		<li class="disabled"><a href="#!"><i class="material-icons">chevron_left</i></a></li>
		<li class="active red"><a href="#!">1</a></li>
		<li class="waves-effect"><a href="#!">2</a></li>
		<li class="waves-effect"><a href="#!">3</a></li>
		<li class="waves-effect"><a href="#!">4</a></li>
		<li class="waves-effect"><a href="#!">5</a></li>
		<li class="waves-effect"><a href="#!"><i class="material-icons">chevron_right</i></a></li>
	</ul>
</div>
<!--
	FIM DA PAGINAÇÃO
-->
<!-- 
	MODAL DE LOGIN 
-->
<div id="LoginModal" class="modal modal-fixed-footer">
	<div class="modal-content">
		<div class="row">
			<div class="center-align">
				<div class="fb-login-button"
					data-auto-logout-link="true"
					data-max-rows="1" 
					data-size="medium" 
					data-button-type="continue_with" 
					data-show-faces="false" 
					onlogin="checkLoginState();" 
					data-auto-logout-link="true" 
					data-button-type="continue_with"
					data-use-continue-as="true"></div>			
			</div>
			<form id="anunciar" class="col s12">
				<div class="row">
					<div class="col s12 l12">
						<h5><span id="beneficios"></span></h5>
					</div>
				</div>
				<div class="row">
					<div class="col s12 l4">
						<p>
							<input type="checkbox" id="clt">
							<label for="clt">C.L.T</label>
						</p>
						<p>
							<input type="checkbox" id="diaria">
							<label for="diaria">Diaria</label>
						</p>
						<p>
							<input type="checkbox" id="odonto">
							<label for="odonto">Plano odontológico</label>
						</p>
						<p>
							<input type="checkbox" id="vida">
							<label for="vida">Seguro de vida</label>
						</p>
					</div>
					<div class="col s12 l4">
						<p>
							<input type="checkbox" id="alimentacao">
							<label for="alimentacao">Vale alimentação</label>
						</p>
						<p>
							<input type="checkbox" id="saude">
							<label for="saude">Plano de saúde</label>
						</p>
						<p>
							<input type="checkbox" id="comissao">
							<label for="comissao">Comissão</label>
						</p>
						<p>
							<input type="checkbox" id="vt">
							<label for="vt">Vale transporte</label>
						</p>										
					</div>
					<div class="input-field col s12 l4">
					    <input placeholder="(00) 00000-0000" id="contact_number" type="text" class="validate">
          				<label for="contact_number">Número de contato</label>
					</div>
					<div class="input-field col s12 l4">
						<select class="icons" id="color">
							<option value="" selected>Escolha uma opção</option>
							<option value="c62828" data-icon="assets/images/c62828.png" class="left circle">Vermelho</option>
							<option value="ad1457" data-icon="assets/images/ad1457.png" class="left circle">Rosa</option>
							<option value="6a1b9a" data-icon="assets/images/6a1b9a.png" class="left circle">Roxo</option>
							<option value="4527a0" data-icon="assets/images/4527a0.png" class="left circle">Roxo escuro</option>
							<option value="283593" data-icon="assets/images/283593.png" class="left circle">Indigo</option>
							<option value="1565c0" data-icon="assets/images/1565c0.png" class="left circle">Azul</option>
							<option value="0277bd" data-icon="assets/images/0277bd.png" class="left circle">Azul claro</option>
							<option value="00838f" data-icon="assets/images/00838f.png" class="left circle">Ciano</option>
							<option value="00695c" data-icon="assets/images/00695c.png" class="left circle">Cerceta</option>
							<option value="2e7d32" data-icon="assets/images/2e7d32.png" class="left circle">Verde</option>
							<option value="558b2f" data-icon="assets/images/558b2f.png" class="left circle">Verde claro</option>
							<option value="9e9d24" data-icon="assets/images/9e9d24.png" class="left circle">Lima</option>
							<option value="f9a825" data-icon="assets/images/f9a825.png" class="left circle">Amarelo</option>
							<option value="ff8f00" data-icon="assets/images/ff8f00.png" class="left circle">Âmbar</option>
							<option value="ef6c00" data-icon="assets/images/ef6c00.png" class="left circle">Laranja</option>
							<option value="d84315" data-icon="assets/images/d84315.png" class="left circle">Laranja escuro</option>
							<option value="4e342e" data-icon="assets/images/4e342e.png" class="left circle">Marron</option>
							<option value="424242" data-icon="assets/images/424242.png" class="left circle">Cinza</option>
							<option value="37474f" data-icon="assets/images/37474f.png" class="left circle">Cinza azulado</option>
						</select>
						<label for="color">Cor do Card</label>
					</div>
				</div>
				<div class="row">
					<div class="col s12 l12">
						<h5><span id="oferecido"></span></h5>
					</div>
				</div>				
				<div class="row">
					<div class="col s12 l4">
						<p>
							<input name="cargo[]" type="radio" id="t1" checked />
							<label for="t1">administrativo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t2" />
							<label for="t2">advogado</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t3" />
							<label for="t3">ajudante de cozinha</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t4" />
							<label for="t4">ajudante geral</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t5" />
							<label for="t5">analista fiscal</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t6" />
							<label for="t6">assistente administrativo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t7" />
							<label for="t7">atendente</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t8" />
							<label for="t8">auxiliar</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t9" />
							<label for="t9">auxiliar administrativo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t10" />
							<label for="t10">auxiliar de enfermagem</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t11" />
							<label for="t11">auxiliar de escritório</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t12" />
							<label for="t13">auxiliar de limpeza</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t13" />
							<label for="t13">auxiliar de produção</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t14" />
							<label for="t14">auxiliar de serviços em geral</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t15" />
							<label for="t15">auxiliar de vendas</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t16" />
							<label for="t16">baba</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t17" />
							<label for="t17">balconista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t18" />
							<label for="t18">bilingue</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t19" />
							<label for="t19">caixa</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t20" />
							<label for="t20">cobrança</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t21" />
							<label for="t21">comercial</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t22" />
							<label for="t22">comprador</label>
						</p>
						<p>
							<input class="wit-gap" name="cargo[]" type="radio" id="t23" />
							<label for="t23">conferente</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t24" />
							<label for="t24">contador</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t25" />
							<label for="25">coordenador</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t26" />
							<label for="t26">copeira</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t27" />
							<label for="t27">costureira</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t28" />
							<label for="t28">cozinheiro</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t29" />
							<label for="t29">demonstradora</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t30" />
							<label for="t30">departamento pessoal</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t31" />
							<label for="t31">designer gráfico</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t32" />
							<label for="t32">diarista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t33" />
							<label for="t33">digitador</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t34" />
							<label for="t34">domestica</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t35" />
							<label for="t35">eletricista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t36" />
							<label for="t36">encanador</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t37" />
							<label for="t37">enfermeiro</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t38" />
							<label for="t38">estoquista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t39" />
							<label for="t39">estágio</label>
						</p>
					</div>
					<div class="col s12 l4">
						<p>
							<input name="cargo[]" type="radio" id="t40" />
							<label for="t40">farmacia</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t41" />
							<label for="t41">farmacêutico</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t42" />
							<label for="t42">faturamento</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t43" />
							<label for="t43">faturista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t44" />
							<label for="t44">financeiro</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t45" />
							<label for="t45">fiscal de loja</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t46" />
							<label for="t46">fisioterapeuta</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t47" />
							<label for="t47">fonoaudíologo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t48" />
							<label for="t48">freelancer</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t49" />
							<label for="t49">frentista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t50" />
							<label for="t50">garçom</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t51" />
							<label for="t51">gerente</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t52" />
							<label for="t52">gerente administrativo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t53" />
							<label for="t53">gerente comercial</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t54" />
							<label for="t54">gerente de loja</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t55" />
							<label for="t55">gerente de vendas</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t56" />
							<label for="t56">grafica</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t57" />
							<label for="t57">home office</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t58" />
							<label for="t58">hospitalar</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t59" />
							<label for="t59">hotelaria</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t60" />
							<label for="t60">hotelaria e turismo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t61" />
							<label for="t61">informatica</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t62" />
							<label for="t62">informatica ti telecomunicações</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t63" />
							<label for="t63">inspetor de alunos</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t64" />
							<label for="t64">inspetor de qualidade</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t65" />
							<label for="t65">jornalista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t66" />
							<label for="t66">juridico</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t67" />
							<label for="t67">laboratorio</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t68" />
							<label for="t68">limpeza</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t69" />
							<label for="t69">logistica</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t70" />
							<label for="t70">manobrista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t71" />
							<label for="t71">marketing</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t72" />
							<label for="t72">mecânico</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t73" />
							<label for="t73">motoboy</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t74" />
							<label for="t74">motorista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t75" />
							<label for="t75">motorista hab b</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t76" />
							<label for="t76">motorista hab d</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t77" />
							<label for="t77">motorista particular</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t78" />
							<label for="t78">nutricionista</label>
						</p>
					</div>
					<div class="col s12 l4">
						<p>
							<input name="cargo[]" type="radio" id="t79" />
							<label for="t79">office boy</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t80" />
							<label for="t80">operacional</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t81" />
							<label for="t81">operador de caixa</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t82" />
							<label for="t82">operador de câmera</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t83" />
							<label for="t83">operador de empilhadeira</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t84" />
							<label for="t84">operador de máquinas</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t85" />
							<label for="t85">operador de produção</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t86" />
							<label for="t86">operador de telemarketing</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t87" />
							<label for="t87">personal trainner</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t88" />
							<label for="t88">pintor</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t89" />
							<label for="t89">porteiro</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t90" />
							<label for="t90">professor</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t91" />
							<label for="t91">programador</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t92" />
							<label for="t92">promotor de vendas</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t93" />
							<label for="t93">químico</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t94" />
							<label for="t94">recepcionista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t95" />
							<label for="t95">recepção</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t96" />
							<label for="t96">recursos humanos</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t97" />
							<label for="t97">repositor</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t98" />
							<label for="t98">representante comercial</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t99" />
							<label for="t99">secretária</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t100" />
							<label for="t100">segurança</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t101" />
							<label for="t101">segurança do trabalho</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t102" />
							<label for="t102">supervisor</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t103" />
							<label for="t103">telefonista</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t104" />
							<label for="t104">telemarketing</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t105" />
							<label for="t105">téc. em ar condicionado</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t106" />
							<label for="t106">téc. em eletrônica</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t107" />
							<label for="t107">téc. em enfermagem</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t108" />
							<label for="t108">téc. em informática</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t109" />
							<label for="t109">téc. em segurança do trabalho</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t110" />
							<label for="t110">usinagem</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t111" />
							<label for="t111">vendedor</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t112" />
							<label for="t112">vendedor de loja</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t113" />
							<label for="t113">vendedor de veículos</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t114" />
							<label for="t114">vendedor externo</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t115" />
							<label for="t115">vendedora</label>
						</p>
						<p>
							<input name="cargo[]" type="radio" id="t116" />
							<label for="t116">vigilante</label>
						</p>									  								
					</div>	      		      	
				</div>
			</form>
		</div>
	</div>
	<div id="modal-foo" class="modal-footer">
		<a href="#!" class="modal-action modal-close waves-effect waves-green btn-flat">Cadastrar</a>
	</div>
</div>
<!-- 
	FINAL DO MODAL DE LOGIN 
-->
<!--
	COMEÇO DO MODAL DE PESQUISA
-->
<div id="SearchModal" class="modal modal-fixed-footer">
	<div class="modal-content">
		<div class="row">
			<form id="search" class="col s12">
				<div class="row">
					<div class="col s12 l12">
						<h5>Categorias</h5>
					</div>
				</div>				
				<div class="row">
					<div class="col s12 l4">
						<p>
							<input name="search" type="radio" id="o1" checked />
							<label for="o1">administrativo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o2" />
							<label for="o2">advogado</label>
						</p>
						<p>
							<input name="search" type="radio" id="o3" />
							<label for="o3">ajudante de cozinha</label>
						</p>
						<p>
							<input name="search" type="radio" id="o4" />
							<label for="o4">ajudante geral</label>
						</p>
						<p>
							<input name="search" type="radio" id="o5" />
							<label for="o5">analista fiscal</label>
						</p>
						<p>
							<input name="search" type="radio" id="o6" />
							<label for="o6">assistente administrativo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o7" />
							<label for="o7">atendente</label>
						</p>
						<p>
							<input name="search" type="radio" id="o8" />
							<label for="o8">auxiliar</label>
						</p>
						<p>
							<input name="search" type="radio" id="o9" />
							<label for="o9">auxiliar administrativo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o10" />
							<label for="o10">auxiliar de enfermagem</label>
						</p>
						<p>
							<input name="search" type="radio" id="o11" />
							<label for="o11">auxiliar de escritório</label>
						</p>
						<p>
							<input name="search" type="radio" id="o12" />
							<label for="o13">auxiliar de limpeza</label>
						</p>
						<p>
							<input name="search" type="radio" id="o13" />
							<label for="o13">auxiliar de produção</label>
						</p>
						<p>
							<input name="search" type="radio" id="o14" />
							<label for="o14">auxiliar de serviços em geral</label>
						</p>
						<p>
							<input name="search" type="radio" id="o15" />
							<label for="o15">auxiliar de vendas</label>
						</p>
						<p>
							<input name="search" type="radio" id="o16" />
							<label for="o16">baba</label>
						</p>
						<p>
							<input name="search" type="radio" id="o17" />
							<label for="o17">balconista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o18" />
							<label for="o18">bilingue</label>
						</p>
						<p>
							<input name="search" type="radio" id="o19" />
							<label for="o19">caixa</label>
						</p>
						<p>
							<input name="search" type="radio" id="o20" />
							<label for="o20">cobrança</label>
						</p>
						<p>
							<input name="search" type="radio" id="o21" />
							<label for="o21">comercial</label>
						</p>
						<p>
							<input name="search" type="radio" id="o22" />
							<label for="o22">comprador</label>
						</p>
						<p>
							<input class="wit-gap" name="search" type="radio" id="o23" />
							<label for="o23">conferente</label>
						</p>
						<p>
							<input name="search" type="radio" id="o24" />
							<label for="o24">contador</label>
						</p>
						<p>
							<input name="search" type="radio" id="o25" />
							<label for="25">coordenador</label>
						</p>
						<p>
							<input name="search" type="radio" id="o26" />
							<label for="o26">copeira</label>
						</p>
						<p>
							<input name="search" type="radio" id="o27" />
							<label for="o27">costureira</label>
						</p>
						<p>
							<input name="search" type="radio" id="o28" />
							<label for="o28">cozinheiro</label>
						</p>
						<p>
							<input name="search" type="radio" id="o29" />
							<label for="o29">demonstradora</label>
						</p>
						<p>
							<input name="search" type="radio" id="o30" />
							<label for="o30">departamento pessoal</label>
						</p>
						<p>
							<input name="search" type="radio" id="o31" />
							<label for="o31">designer gráfico</label>
						</p>
						<p>
							<input name="search" type="radio" id="o32" />
							<label for="o32">diarista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o33" />
							<label for="o33">digitador</label>
						</p>
						<p>
							<input name="search" type="radio" id="o34" />
							<label for="o34">domestica</label>
						</p>
						<p>
							<input name="search" type="radio" id="o35" />
							<label for="o35">eletricista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o36" />
							<label for="o36">encanador</label>
						</p>
						<p>
							<input name="search" type="radio" id="o37" />
							<label for="o37">enfermeiro</label>
						</p>
						<p>
							<input name="search" type="radio" id="o38" />
							<label for="o38">estoquista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o39" />
							<label for="o39">estágio</label>
						</p>
					</div>
					<div class="col s12 l4">
						<p>
							<input name="search" type="radio" id="o40" />
							<label for="o40">farmacia</label>
						</p>
						<p>
							<input name="search" type="radio" id="o41" />
							<label for="o41">farmacêutico</label>
						</p>
						<p>
							<input name="search" type="radio" id="o42" />
							<label for="o42">faturamento</label>
						</p>
						<p>
							<input name="search" type="radio" id="o43" />
							<label for="o43">faturista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o44" />
							<label for="o44">financeiro</label>
						</p>
						<p>
							<input name="search" type="radio" id="o45" />
							<label for="o45">fiscal de loja</label>
						</p>
						<p>
							<input name="search" type="radio" id="o46" />
							<label for="o46">fisioterapeuta</label>
						</p>
						<p>
							<input name="search" type="radio" id="o47" />
							<label for="o47">fonoaudíologo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o48" />
							<label for="o48">freelancer</label>
						</p>
						<p>
							<input name="search" type="radio" id="o49" />
							<label for="o49">frentista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o50" />
							<label for="o50">garçom</label>
						</p>
						<p>
							<input name="search" type="radio" id="o51" />
							<label for="o51">gerente</label>
						</p>
						<p>
							<input name="search" type="radio" id="o52" />
							<label for="o52">gerente administrativo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o53" />
							<label for="o53">gerente comercial</label>
						</p>
						<p>
							<input name="search" type="radio" id="o54" />
							<label for="o54">gerente de loja</label>
						</p>
						<p>
							<input name="search" type="radio" id="o55" />
							<label for="o55">gerente de vendas</label>
						</p>
						<p>
							<input name="search" type="radio" id="o56" />
							<label for="o56">grafica</label>
						</p>
						<p>
							<input name="search" type="radio" id="o57" />
							<label for="o57">home office</label>
						</p>
						<p>
							<input name="search" type="radio" id="o58" />
							<label for="o58">hospitalar</label>
						</p>
						<p>
							<input name="search" type="radio" id="o59" />
							<label for="o59">hotelaria</label>
						</p>
						<p>
							<input name="search" type="radio" id="o60" />
							<label for="o60">hotelaria e turismo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o61" />
							<label for="o61">informatica</label>
						</p>
						<p>
							<input name="search" type="radio" id="o62" />
							<label for="o62">informatica ti telecomunicações</label>
						</p>
						<p>
							<input name="search" type="radio" id="o63" />
							<label for="o63">inspetor de alunos</label>
						</p>
						<p>
							<input name="search" type="radio" id="o64" />
							<label for="o64">inspetor de qualidade</label>
						</p>
						<p>
							<input name="search" type="radio" id="o65" />
							<label for="o65">jornalista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o66" />
							<label for="o66">juridico</label>
						</p>
						<p>
							<input name="search" type="radio" id="o67" />
							<label for="o67">laboratorio</label>
						</p>
						<p>
							<input name="search" type="radio" id="o68" />
							<label for="o68">limpeza</label>
						</p>
						<p>
							<input name="search" type="radio" id="o69" />
							<label for="o69">logistica</label>
						</p>
						<p>
							<input name="search" type="radio" id="o70" />
							<label for="o70">manobrista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o71" />
							<label for="o71">marketing</label>
						</p>
						<p>
							<input name="search" type="radio" id="o72" />
							<label for="o72">mecânico</label>
						</p>
						<p>
							<input name="search" type="radio" id="o73" />
							<label for="o73">motoboy</label>
						</p>
						<p>
							<input name="search" type="radio" id="o74" />
							<label for="o74">motorista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o75" />
							<label for="o75">motorista hab b</label>
						</p>
						<p>
							<input name="search" type="radio" id="o76" />
							<label for="o76">motorista hab d</label>
						</p>
						<p>
							<input name="search" type="radio" id="o77" />
							<label for="o77">motorista particular</label>
						</p>
						<p>
							<input name="search" type="radio" id="o78" />
							<label for="o78">nutricionista</label>
						</p>
					</div>
					<div class="col s12 l4">
						<p>
							<input name="search" type="radio" id="o79" />
							<label for="o79">office boy</label>
						</p>
						<p>
							<input name="search" type="radio" id="o80" />
							<label for="o80">operacional</label>
						</p>
						<p>
							<input name="search" type="radio" id="o81" />
							<label for="o81">operador de caixa</label>
						</p>
						<p>
							<input name="search" type="radio" id="o82" />
							<label for="o82">operador de câmera</label>
						</p>
						<p>
							<input name="search" type="radio" id="o83" />
							<label for="o83">operador de empilhadeira</label>
						</p>
						<p>
							<input name="search" type="radio" id="o84" />
							<label for="o84">operador de máquinas</label>
						</p>
						<p>
							<input name="search" type="radio" id="o85" />
							<label for="o85">operador de produção</label>
						</p>
						<p>
							<input name="search" type="radio" id="o86" />
							<label for="o86">operador de telemarketing</label>
						</p>
						<p>
							<input name="search" type="radio" id="o87" />
							<label for="o87">personal trainner</label>
						</p>
						<p>
							<input name="search" type="radio" id="o88" />
							<label for="o88">pintor</label>
						</p>
						<p>
							<input name="search" type="radio" id="o89" />
							<label for="o89">porteiro</label>
						</p>
						<p>
							<input name="search" type="radio" id="o90" />
							<label for="o90">professor</label>
						</p>
						<p>
							<input name="search" type="radio" id="o91" />
							<label for="o91">programador</label>
						</p>
						<p>
							<input name="search" type="radio" id="o92" />
							<label for="o92">promotor de vendas</label>
						</p>
						<p>
							<input name="search" type="radio" id="o93" />
							<label for="o93">químico</label>
						</p>
						<p>
							<input name="search" type="radio" id="o94" />
							<label for="o94">recepcionista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o95" />
							<label for="o95">recepção</label>
						</p>
						<p>
							<input name="search" type="radio" id="o96" />
							<label for="o96">recursos humanos</label>
						</p>
						<p>
							<input name="search" type="radio" id="o97" />
							<label for="o97">repositor</label>
						</p>
						<p>
							<input name="search" type="radio" id="o98" />
							<label for="o98">representante comercial</label>
						</p>
						<p>
							<input name="search" type="radio" id="o99" />
							<label for="o99">secretária</label>
						</p>
						<p>
							<input name="search" type="radio" id="o100" />
							<label for="o100">segurança</label>
						</p>
						<p>
							<input name="search" type="radio" id="o101" />
							<label for="o101">segurança do trabalho</label>
						</p>
						<p>
							<input name="search" type="radio" id="o102" />
							<label for="o102">supervisor</label>
						</p>
						<p>
							<input name="search" type="radio" id="o103" />
							<label for="o103">telefonista</label>
						</p>
						<p>
							<input name="search" type="radio" id="o104" />
							<label for="o104">telemarketing</label>
						</p>
						<p>
							<input name="search" type="radio" id="o105" />
							<label for="o105">téc. em ar condicionado</label>
						</p>
						<p>
							<input name="search" type="radio" id="o106" />
							<label for="o106">téc. em eletrônica</label>
						</p>
						<p>
							<input name="search" type="radio" id="o107" />
							<label for="o107">téc. em enfermagem</label>
						</p>
						<p>
							<input name="search" type="radio" id="o108" />
							<label for="o108">téc. em informática</label>
						</p>
						<p>
							<input name="search" type="radio" id="o109" />
							<label for="o109">téc. em segurança do trabalho</label>
						</p>
						<p>
							<input name="search" type="radio" id="o110" />
							<label for="o110">usinagem</label>
						</p>
						<p>
							<input name="search" type="radio" id="o111" />
							<label for="o111">vendedor</label>
						</p>
						<p>
							<input name="search" type="radio" id="o112" />
							<label for="o112">vendedor de loja</label>
						</p>
						<p>
							<input name="search" type="radio" id="o113" />
							<label for="o113">vendedor de veículos</label>
						</p>
						<p>
							<input name="search" type="radio" id="o114" />
							<label for="o114">vendedor externo</label>
						</p>
						<p>
							<input name="search" type="radio" id="o115" />
							<label for="o115">vendedora</label>
						</p>
						<p>
							<input name="search" type="radio" id="o116" />
							<label for="o116">vigilante</label>
						</p>									  								
					</div>	      		      	
				</div>
			</form>
		</div>
	</div>
	<div id="modal-foo" class="modal-footer">
		<a href="#!" class="modal-action modal-close waves-effect waves-green btn-flat">Buscar</a>
	</div>
</div>
<!--
	FIM DO MODAL DE PESQUISA
-->	